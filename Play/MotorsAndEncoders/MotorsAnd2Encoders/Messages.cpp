...
PC_EncoderCountsMessage::PC_EncoderCountsMessage ()
{
...
PC_EncoderCountsMessage::PC_EncoderCountsMessage (byte msgBytes []) : header (msgBytes)
{
   int get = offsetof (PC_EncoderCountsMessage, data);
...
bool PC_EncoderCountsMessage::Add (byte e1, byte e2)
{
...
bool PC_EncoderCountsMessage::ToBytes (byte byteArray [], int maxLength)
{
   if (maxLength < sizeof (StatusMessage))
       return false;
...
void PC_EncoderCountsMessage::ToString () // prints to console, not a character array
{
   header.ToString ();
